...
class SteamStoreClient(BaseScraper):
    ...
    def __init__(self):
        super().__init__(
            rate_limit_requests=1, rate_limit_period=1.5  # 1.5 seconds between batch requests
        )
    ...
    async def fetch_batch(
        self, appids: List[int], batch_size: int = 200, country_code: str = "us"
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(0, len(appids), batch_size):
            ...
            logger.info(f"Fetching Steam Store batch {i // batch_size + 1} " f"({len(batch)} apps)")

            for appid in batch:
                data = await self.fetch_app_details(appid, country_code)
                if data:
                    all_data.append({"appid": appid, "data": data})
            ...
            try:
                ...
                if not price_info:
                    ...
                    parsed_record = {
                        "appid": appid,
                        "is_free": False,
                        "current_price": price_info.get("final", 0)
                ...
                parsed_record.update(
                    {
                        ...
                        "categories": [
                            c.get("description", "") for c in app_info.get("categories", [])
                        ],
                    }
                )
    ...
    def transform(self, parsed_data: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for record in parsed_data:
            ...
            game_update = {
                ...
                "developer": ", ".join(record.get("developers", []))[:500]
                if record.get("developers")
                else None,
                "publisher": ", ".join(record.get("publishers", []))[:500]
