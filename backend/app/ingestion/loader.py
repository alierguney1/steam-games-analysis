...
from app.db.models import (
    DimGame, DimGenre, DimTag, DimDate,
    FactPlayerPrice, BridgeGameTag
)
...
class DataLoader:
    ...
    async def load_genres(self, genres: List[Dict[str, Any]]) -> int:
        ...
        for genre in genres:
            ...
            stmt = pg_insert(DimGenre).values(
    ...
    async def load_facts(self, facts: List[Dict[str, Any]]) -> int:
        ...
        for fact in facts:
            ...
            date_id = await self._get_or_create_date_id(
                fact.get("year"),
                fact.get("month")
            )
            if not date_id:
    ...
    async def load_bridges(self, bridges: List[Dict[str, Any]]) -> int:
        ...
        for bridge in bridges:
            ...
            stmt = pg_insert(BridgeGameTag).values(
                game_id=game_id,
                tag_id=tag_id
    ...
    async def _get_game_id(self, appid: int) -> Optional[int]:
        ...
        result = await self.session.execute(
            select(DimGame.game_id).where(DimGame.appid == appid)
        )
        row = result.first()
    ...
    async def _get_or_create_date_id(self, year: Optional[int], month: Optional[int]) -> Optional[int]:
        """
