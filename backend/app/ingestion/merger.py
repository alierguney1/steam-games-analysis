...
class DataMerger:
    ...
    def merge_game_data(
        ...
        logger.info(
            f"Merge complete: {len(merged_games)} games, "
    ...
    def _merge_game_metadata(
        self, steamspy_games: List[Dict[str, Any]], steam_store_updates: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if not df_store.empty:
            ...
            df_merged = df_steamspy.merge(df_store, on="appid", how="left", suffixes=("", "_store"))
    ...
    def _merge_fact_records(
        self, steamcharts_facts: List[Dict[str, Any]], steam_store_pricing: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        df_merged = df_charts.merge(df_pricing, on="appid", how="left")
    ...
    def _create_bridge_records(self, steamspy_raw: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for game in steamspy_raw:
            ...
            for tag_name in tags.keys():
                bridge_records.append({"appid": appid, "tag_name": tag_name})

        return bridge_records

    def deduplicate_facts(self, fact_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
